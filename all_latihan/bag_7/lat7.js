...
function binarySearch(producsDetail, target) {
  let left = 0;
  let right = producsDetail.length - 1;

    while (left <= right) {
        const mid = Math.floor((left + right) / 2);
        if (producsDetail[mid].id === target) {
            return mid;
        } else if (producsDetail[mid].id < target) {
            left = mid + 1; 
        } else {
            right = mid - 1;
        }
...
// console.log(binarySearch(producsDetail, 5));

const sortedProducts = [...producsDetail].sort((a, b) => a.price - b.price);

function binarySearchPrice(sortedProducts, targetPrice) {
  let left = 0;
...
const resultFound = binarySearchPrice(sortedProducts, 950);
console.log("Ditemukan:", resultFound.title); 
// Output: Ditemukan: Convertible 2-in-1 Touch

const resultNotFound = binarySearchPrice(sortedProducts, 500);
console.log("Ditemukan:", resultNotFound); 
// Output: Ditemukan: undefined