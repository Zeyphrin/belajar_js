...
function searchProduct(keyword) {
  ...
  console.log(`\nMencari: "${keyword}"`);
  console.log(`Riwayat saat ini:`, searchHistory.items);
}

function undoSearch() {
  if (searchHistory.isEmpty()) {
    console.log("\nTidak ada riwayat pencarian yang bisa di-undo.");
    return;
  }

  const removedSearch = searchHistory.pop();
  console.log(`\n[Undo] Menghapus pencarian: "${removedSearch}"`);

  if (!searchHistory.isEmpty()) {
    const previousSearch = searchHistory.peek();
    console.log(`=> Kembali menampilkan hasil untuk: "${previousSearch}"`);
  } else {
    console.log("=> Riwayat kosong. Kembali ke halaman utama.");
  }
}
