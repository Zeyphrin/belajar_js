...
const titles = producsDetail.map(p => p.title);
const expensiveproducs = producsDetail.filter(p => p.price > 500);
const totalStock = producsDetail.reduce((sum, p) => sum + p.stock, 0);


const laptopPrices = producsDetail
...
function getStatistics(producsDetail) {
  const totalproducs = producsDetail.length;


  const totalStock = producsDetail.reduce((sum, p) => sum + p.stock, 0);
  const totalPrice = producsDetail.reduce((sum, p) => sum + p.price, 0);
  const totalRating = producsDetail.reduce((sum, p) => sum + (p.rating || 0), 0);

  const averagePrice = Number((totalPrice / totalproducs).toFixed(2));
  const averageRating = Number((totalRating / totalproducs).toFixed(2));

  const prices = producsDetail.map(p => p.price);
...
console.log(getStatistics(producsDetail));