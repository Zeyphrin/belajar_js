...
function countFrequency(productsDetail) {
  return productsDetail.reduce((counts, item) => {
...
const categories = productsDetail.map(p => p.category);
console.log("Frekuensi Kategori:", countFrequency(categories));

const allTags = productsDetail.flatMap(p => p.tags);
console.log("Frekuensi Tags:", countFrequency(allTags));

const roundedRatings = productsDetail.map(p => Math.round(p.rating));
console.log("Frekuensi Rating:", countFrequency(roundedRatings));

const brands = productsDetail.map(p => p.brand).filter(brand => brand !== undefined);
console.log("Frekuensi Brand:", countFrequency(brands));